...
async function updateClientData() {
  try {
    ...
    // Primero, vamos a ver algunos ejemplos de emails del CSV
    console.log('\n📋 Primeros 5 emails del CSV:');
    clients.slice(0, 5).forEach((client, index) => {
      console.log(`${index + 1}. ${client.email}`);
    });

    // Verificar algunos emails en la base de datos
    const dbEmails = await pool.query('SELECT email FROM clients LIMIT 5');
    console.log('\n📋 Primeros 5 emails de la base de datos:');
    dbEmails.rows.forEach((row, index) => {
